...
//
    ...
    final class Event_SMPTEOffset_Tests: XCTestCase {
        func testInit() {
            let event = MIDIFile.Event.SMPTEOffset(hr: 1, min: 2, sec: 3, fr: 4, subFr: 5, frRate: ._25fps)
        ...
        func testInit_rawData() {
            let rawData: [UInt8] = [0xFF, 0x54, 0x05,
                                    0b0010_0001, 2, 3, 4, 5]

            let event = try! MIDIFile.Event.SMPTEOffset(rawData: rawData)
        ...
        func testInit_Timecode() {
            ...
            do {
                ...
                let smpte = MIDIFile.Event.SMPTEOffset(scaling: tc)
