...
extension MIDI.File.Event {
    ...
    /// It is used to specify the SMPTE time at which the track is to start. Defaults to 00:00:00:00.00, 24fps.
    ///
    ...
    public struct SMPTEOffset: Equatable, Hashable {
        ...
        init(hr: UInt8,
             ...
             fr: UInt8,
             subFr: UInt8 = 0,
             frRate: MIDI.File.SMPTEOffsetFrameRate = ._30fps) {
        ...
        #warning("> TODO: add an init from Timecode struct that can convert/scale timecode and subframes to 100 subframe divisor")
...
extension MIDI.File.Event.SMPTEOffset: MIDIFileEvent {
    ...
    public init(midi1SMFRawBytes rawBytes: [MIDI.Byte]) throws {
        ...
        // 3-byte preamble
        guard rawBytes.starts(with: MIDI.File.kEventHeaders[.smpteOffset]!) else {
            throw MIDI.File.DecodeError.malformed(
        ...
        guard let readFrameRate = MIDI.File.SMPTEOffsetFrameRate(rawValue: readFrameRateBits) else {
...
extension Timecode {
    ...
    ///
    /// MIDI SMPTE Offset subframes (fractional frames) are always in 100ths of a frame, even in SMPTE- based tracks which specify a different frame subdivision for delta-times.
    public var scaledToMIDIFileSMPTEFrameRate: (scaledTimecode: Timecode?,
                                                smpteFR: MIDI.File.SMPTEOffsetFrameRate) {
           ...
           let nonNilscaledTC = scaledTC
        {
            ...
            scaledTC = try? newComponents.toTimecode(at: nonNilscaledTC.frameRate,
                                                     base: ._100SubFrames)
        }
...
extension Timecode.FrameRate {
    ...
    public var midiFileSMPTEOffsetRate: MIDI.File.SMPTEOffsetFrameRate {
...
extension MIDI.File.SMPTEOffsetFrameRate {
